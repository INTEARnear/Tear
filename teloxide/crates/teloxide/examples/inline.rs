...
async fn main() {
    ...
    let handler =
        Update::filter_inline_query().branch(dptree::endpoint(
            |bot: Bot, q: InlineQuery| async move {
                ...
                // While constructing them from the struct itself is possible,
                // it is preferred to use the builder pattern if
                // you wish to add more information to your
                // result. Please refer to the documentation for more detailed information about each field. https://docs.rs/teloxide/latest/teloxide/types/struct.InlineQueryResultArticle.html
                ...
                .thumbnail_url(
                    "https://duckduckgo.com/assets/logo_header.v108.png".parse().unwrap(),
                )
                ...
                // Send it off! One thing to note -- the ID we use here must be
                // of the query we're responding to.
