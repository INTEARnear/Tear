            ...
            TgCommand::NearTgiAnswer(command, answer) => {
                            ...
                            Err(js_err) => {
                                let error_message = match err.kind() {
                                    ErrorKind::DisplayVersion => {
                                        format!(
                                            "near-tgi, implementing near-cli-rs {}",
                                            env!("NEAR_CLI_VERSION")
                ...
                tokio::task::spawn_blocking(move || {
                    let result = std::panic::catch_unwind(|| {
                            ...
                            ResultFromCli::Err(optional_cli_cmd, err) => {
                                if err.to_string() != "The input device is not a TTY" {
                                    if let Some(cli_cmd) = optional_cli_cmd {
                                        ...
                                        let error = if let Some((before, after)) =
                                            error.split_once("Location:\n    ")
                                        {
                                    ...
                                    CURRENT_PROMPT.with(|prompt| prompt.borrow_mut().take())
                                {
                                    if let Some(cli_cmd) = optional_cli_cmd {
                                        command_string = shell_words::join(
                                            std::iter::once("near".to_string())
                        ...
                        (
                            command_string,
                            ResponseOrPrompt::Response(
                                LOG_COLLECTOR
                                    .with(|logger| logger.borrow_mut().drain_logs())
                                    .join("\n"),
                            ),
                        )
                    });
                    let (command_string, response) = match result {
                        Ok((command_string, response)) => (command_string, response),
                        Err(_) => {
                            log::info!("Catched a panic");
